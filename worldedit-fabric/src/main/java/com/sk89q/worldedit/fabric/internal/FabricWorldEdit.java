...
public class FabricWorldEdit extends CoreMcMod implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        FabricAdapterBootstrap.init();
        new FaweFabric(this);

        try {
            CUIPacketHandler.instance().registerServerboundHandler(this::onCuiPacket);
        } catch (NoClassDefFoundError e) {
            LOGGER.warn("WorldEditCUI protocol not available; selection outlines disabled", e);
        }
        ...
        ServerLifecycleEvents.SERVER_STARTING.register(server -> {
            // FAWE queue sync tasks require the Minecraft server thread as "main".
            com.fastasyncworldedit.core.Fawe.instance().setMainThread();
            FabricTaskManager.setServer(server);
            com.sk89q.worldedit.fabric.adapter.v26_2.FabricWorlds.setServer(server);
            serverAboutToStart();
        });
        ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            serverStarted(server);
            // Preferences may not exist yet if command registration has not run; force choosePreferred.
            WorldEdit.getInstance().getEventBus().post(new PlatformsRegisteredEvent());
            // FAWE BlockTypesCache must init after platform registries are ready (uses BlockRegistry.values()).
            BlockTypes.AIR.getDefaultState();
            if (BlockTypesCache.states.length == 0) {
                LOGGER.warn("BlockTypesCache initialized with no block states");
            }
            FabricAdapterBootstrap.getAdapter().ensureInit();
        });
        ...
        LOGGER.info("FastAsyncWorldEdit for Fabric (version " + getInternalVersion() + ") is loaded");
    }
